...
final class AppContainer {
    ...
    /// Opt-in read-only usage sync to a paired phone over the local network. See `LANSyncServer`.
    ...
    /// The LAN-reachable counterpart of `localAPI`, gated by `lanSync.enabled` (default off).
    private let lanSyncServer: LANSyncServer
    ...
    init(isFreshInstall: Bool = false) {
        ...
        let lanSyncServer = LANSyncServer(
            store: lanSync,
            usageState: { [layout, enablement, registry, dataStore] in
                // Deliberately stricter than the loopback API's `enabledOrderedIDs` (that one is
                // machine-facing and keeps every enabled provider regardless of layout). The phone
                // is a mirror of what the dashboard actually shows a human, so a provider whose
                // toggle is on but whose every metric is hidden in Customize — visible to
                // `enablement` but invisible in `layout.displayGroups` — must stay off the phone too.
                let visibleProviderIDs = Set(layout.displayGroups.map(\.provider.id))
                return LocalUsageAPI.State(
                    enabledOrderedIDs: layout.orderedProviderIDs()
                        .filter { enablement.isEnabled($0) && visibleProviderIDs.contains($0) },
                    knownIDs: Set(registry.providers.map(\.id)),
                    snapshots: dataStore.snapshots,
                    limitDescriptors: registry.limitDescriptorsByProvider,
                    errors: dataStore.providerErrors
        ...
        // Weak on the server side only: `lanSync.onEnabledChange` must not keep `lanSyncServer` (and
        // through it, `lanSync` itself via its `store` reference) alive forever.
        lanSync.onEnabledChange = { [weak lanSyncServer] enabled in
            if enabled { lanSyncServer?.start() } else { lanSyncServer?.stop() }
        }
        if lanSync.enabled { lanSyncServer.start() }
        self.lanSyncServer = lanSyncServer
    ...
    func resetAllSettings() {
        ...
        // Turns off the LAN listener and forgets every paired phone; a phone must be re-paired with
        // a fresh QR code afterward.
        lanSync.resetToDefaults()
