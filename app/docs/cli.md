...
MaxUsage ships a one-shot `maxusage` command for agents and scripts. It prints the documented
...
```sh
maxusage                 # every enabled provider, refreshing stale cache entries
maxusage codex           # one provider, refreshing when its cache is stale
maxusage codex --force   # refresh through the shared provider engine, cache, print, exit
```
...
In MaxUsage, open **Settings → Command Line** and click **Install…**. After the standard macOS
administrator prompt, `maxusage` is available globally in new terminal sessions. The installed symlink
points to the helper inside MaxUsage, so replacing the app also updates the command.
