...
```sh
log stream --info --style compact --predicate 'process == "MaxUsage"'
```

To read logs *after the fact* instead of live, use `log show` with a time window:

```sh
log show --last 10m --info --predicate 'process == "MaxUsage"'
```

## Log file

In addition to the unified log above, the app writes a file log to
`~/Library/Logs/MaxUsage/MaxUsage.log` — this is what to send with a support report. It is capped at
