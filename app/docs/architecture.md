...
A high-level map of how MaxUsage is put together, for people working on the code. For what the app
*does*, start with the [behavior docs](README.md).

## The shape of the app

MaxUsage is a SwiftPM package with a shared module and two thin executables — there is no Xcode project.
...
The `maxusage` executable imports the same module. A normal invocation reads `ProviderSnapshotCache`
...
1. **Auth store** — reads credentials that already exist on the machine (config files, keychain). MaxUsage
...
macOS menu-bar apps live in an `NSStatusItem`. MaxUsage shows its content in a custom, key-capable
...
MaxUsage runs on macOS 15 (Sequoia) and later. It is built against the latest SDK and back-deploys:
...
The release build (`script/package_release.sh`) produces a universal arm64 + x86_64 binary and packages
an ad-hoc signed DMG. The dev build (`script/build_and_run.sh`) stays host-architecture only to keep local
iteration fast. Developer ID signing and notarization can be layered onto the same universal package later.
