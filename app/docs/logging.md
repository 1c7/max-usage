...
MaxUsage keeps a file log so you can capture what the app was doing and share it with support when
...
```
~/Library/Logs/MaxUsage/MaxUsage.log
```
...
If a local usage log exists but cannot be read, MaxUsage writes one warning and skips it for that
...
```sh
grep '\[refresh\]' ~/Library/Logs/MaxUsage/MaxUsage.log
```
...
The log is capped at ~10 MB. When it fills up, the current file is rotated to `MaxUsage.1.log` and a
fresh `MaxUsage.log` starts, so a long-running session can never fill your disk (at most ~20 MB across
the live file and one archive). An oversize file left over from a previous session is rotated once at
launch.

> Note: the dev build and a released build both write to the same `MaxUsage.log`. Running them at the
