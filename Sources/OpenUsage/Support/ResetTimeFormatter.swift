...
enum ResetTimeFormatter {
    ...
    static func format(hoursUntilReset hours: Double) -> String {
        if hours < 1.0 / 60.0 {
            return String(localized: "resetTime.minutes", defaultValue: "Resets in \(1) min")
        }
        if hours < 1 {
            let minutes = Int(max(1, (hours * 60).rounded()))
            return String(localized: "resetTime.minutes", defaultValue: "Resets in \(minutes) min")
        }
        if hours < 24 {
            let rounded = Int(hours.rounded())
            if rounded >= 24 {
                return String(localized: "resetTime.tomorrow", defaultValue: "Resets tomorrow")
            }
            return String(localized: "resetTime.hours", defaultValue: "Resets in \(rounded) hr")
        }
        if hours < 48 {
            return String(localized: "resetTime.tomorrow", defaultValue: "Resets tomorrow")
        }
        let days = max(1, Int((hours / 24).rounded()))
        return String(localized: "resetTime.days", defaultValue: "Resets in \(days) days")
    }
}
