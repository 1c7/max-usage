...
When nothing currently qualifies for a recommendation (see [Dashboard § Recommended](dashboard.md#recommended) for when that happens), the tray falls back to the plain MaxUsage icon.
...
Settings → Privacy → **Hide From Screen Share** (off by default). While your screen is being shared or recorded — a Zoom/Meet/Teams share, a screen recording, macOS Screen Sharing — the strip is replaced with the MaxUsage icon and wordmark, so quota numbers never show up in front of an audience. The moment the capture ends, the recommendation comes right back. Captures you start yourself (a screen recording, for example) count too, so those get the wordmark as well.
...
![The menu bar strip concealed behind the MaxUsage wordmark](assets/menu-bar-privacy-sharing.png)
