...
MaxUsage exposes a read-only HTTP API on the loopback interface so other local apps can consume the same usage data shown in the menu bar.
...
and the exact format printed by the `maxusage` CLI.
