...
- The one-shot `maxusage` command reuses this same persisted cache for five minutes, refreshes missing or stale entries without starting the app, and exits. `maxusage --force` runs the same forced provider refresh as ⌘R regardless of cache age.
