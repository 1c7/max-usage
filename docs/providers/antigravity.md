...
MaxUsage never asks for a token — it reads what Antigravity already has:

- **Antigravity running** — MaxUsage talks to the app's local language server (the richest source, and where the plan name comes from).
- **App closed** — it falls back to the OAuth token Antigravity / `agy` store in your macOS Keychain and queries Google's Cloud Code API. An expired token is refreshed automatically (MaxUsage never writes back to Antigravity's own keychain item). Its short-lived cache is reused only while the same Keychain login is present and readable.
...
- **"Couldn't read Antigravity credentials…"** — unlock Keychain or sign in to Antigravity again. MaxUsage will not use its cached access token until the current login can be verified.
