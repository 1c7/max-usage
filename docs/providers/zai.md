...
When Z.ai reports your plan name, MaxUsage shows it beside the provider name.

## Where credentials come from

Z.ai has no companion CLI/app that MaxUsage can reuse a credential from, so you supply an API key.
MaxUsage reads it from the first place it finds one, in this order:
...
2. Add the key to MaxUsage via **Settings → API Keys**, **or** export it:
